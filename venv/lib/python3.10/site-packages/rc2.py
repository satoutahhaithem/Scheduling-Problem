...
# Backporting Python 2.7.x to Python 3 Code by koha <kkoha@msn.com>
...
class RC2():
    ...
    def __init__(self, keys):
        ...
        self.K.fromstring(bytes(rc2_key))
    ...
    def block_encrypt(self, input_buffer):
        ...
        R = array.array('H')
        R.fromstring(bytes(input_buffer))
    ...
    def encrypt(self, input_buffer, mode, IV = None, padding = None):
        ...
        input_buffer = bytearray(input_buffer)
            ...
            if block_counter == 0:
                ...
                if mode == MODE_CBC:
                    for i in range(RC2_BLOCK_SIZE):
                        ...
                        block[i] = int(block[i]) ^ ord(block_result[i])
        ...
        for block_counter in range(crypt_size//RC2_BLOCK_SIZE):
            ...
            if mode == MODE_CBC:
                if block_counter == 0:
                    if IV is not None:
                        for i in range(RC2_BLOCK_SIZE):
                            crypt_buffer[block_counter * RC2_BLOCK_SIZE + i] = ord(block_result[i]) ^ IV[i]
                else:
                    for i in range(RC2_BLOCK_SIZE):
                        crypt_buffer[block_counter * RC2_BLOCK_SIZE + i] = ord(block_result[i]) ^ previous_block[i]
            else:
                for i in range(RC2_BLOCK_SIZE):
                    crypt_buffer[block_counter * RC2_BLOCK_SIZE + i] = ord(block_result[i])
